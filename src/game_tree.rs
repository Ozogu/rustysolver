...
impl<G: Game> GameTree<G> {
    ...
    pub fn build(&mut self) {
        ...
        self.game.generate_deals().iter().for_each(|deal| {
            ...
            self.iterate_tree(node, &mut builder_visitor);
        });
    ...
    fn iterate_tree<V: Visitor>(&mut self, node: Node, visitor: &mut V) {
        if node.is_terminal(&self.game) {
            ...
            for card in node.deck.iter() {
                ...
                self.iterate_tree(next_node, visitor);
            }
            ...
            for action in node.actions.iter() {
                ...
                self.iterate_tree(new_node, visitor);
            }
        }
    }
}
